...
export const getImportPath = async (_isGetContext?: boolean, edition?: TOOLJET_EDITIONS) => {
  const repoType = edition || getTooljetEdition() || TOOLJET_EDITIONS.CE;
  ...
  switch (repoType) {
    case TOOLJET_EDITIONS.CE:
      baseDir = isDist ? join('dist', 'src', 'modules') : join('src', 'modules');
      return join(process.cwd(), baseDir);
    case TOOLJET_EDITIONS.EE:
    case TOOLJET_EDITIONS.Cloud:
      baseDir = isDist ? join('dist', 'ee') : 'ee';
      return join(process.cwd(), baseDir);
    default:
      baseDir = isDist ? join('dist', 'src', 'modules') : join('src', 'modules');
      return join(process.cwd(), baseDir);
